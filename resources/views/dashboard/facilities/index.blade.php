...
@section('title'){!! __('Facilities') !!}@endsection
@section('header')
...
@section('btn')
    @can('add facilities')
        @include('dashboard.layouts.partials._add_icon',['route'=>'facilities'])
    @endcan
@endsection
@section('content')
...
{{--                        <h4 class="card-title">@yield('title')</h4>--}}
                    ...
                    <div class="card-content">
                        <div class="card-body card-dashboard">
                                ...
                                <div class="card-content collapse @if(count(request()->except('type')) > 0) show @endif
                                 ">
                                    <div class="card-body card-dashboard">
                                        ...
                                        <div class="row">
                                            @include('dashboard.facilities.partials._form_filter')
                                            <div class="col-12">
                                                <button type="submit" class="btn btn-primary   waves-effect waves-light btn-sm">{{__('Filter')}}</button>
                                                <a href="{{route('dashboard.facilities.index')}}" class="  ml-1 btn btn-warning  waves-effect waves-light btn-sm">{{__('Reset filter')}}</a>
                                            </div>
                                        </div>
                                        {!! Form::close() !!}
                                    </div>
                            ...
                            <div class="row">
                                @include('dashboard.facilities.partials._table')
                            </div>
                        </div>
                    </div>
                </div>
