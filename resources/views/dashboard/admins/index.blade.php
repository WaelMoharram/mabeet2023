                        ...
                        <div class="card-body card-dashboard">
                            <div class="card ">
                                <div class="card-header">
                                    <div class="col-6">
                                        <a href="{{route('dashboard.admins.index')}}" class="btn  @if(!request()->has('role')) btn-success @else btn-outline-success @endif round waves-effect">{{__('All')}}</a>
                                        @foreach(\Spatie\Permission\Models\Role::pluck('name','id') as $role)
                                            <a href="{{route('dashboard.admins.index',['role'=>$role])}}" class="btn @if(request()->has('role') && request()->role == $role) btn-success @else btn-outline-success @endif round waves-effect">{{$role}}</a>
                                        @endforeach
                                    </div>
