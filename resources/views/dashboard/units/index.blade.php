...
@section('title'){!! __('Units') !!}@endsection
@section('header')
...
{{--                        <h4 class="card-title">@yield('title')</h4>--}}
                        <div class="col-6">
                            <a href="{{route('dashboard.units.index')}}" class="btn  @if(!request()->has('unit_type_id')) btn-success @else btn-outline-success @endif round waves-effect">{{__('All')}}</a>
                            @foreach(\App\Models\UnitType::all() as $type)
                                <a href="{{route('dashboard.units.index',['unit_type_id'=>$type->id])}}" class="btn @if(request()->has('unit_type_id') && request()->unit_type_id == $type->id) btn-success @else btn-outline-success @endif round waves-effect">{{$type->name}}</a>
                            @endforeach
                        </div>
